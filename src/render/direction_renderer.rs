...
pub fn draw_north_left(c: Context, g: &mut G2d) {
    // Draw the straight line from the bottom to the curve's starting point.
    line(WHITE, 1.0, [WINDOW_WIDTH / 2.0 + 50.0, WINDOW_HEIGHT, WINDOW_WIDTH / 2.0 + 50.0, WINDOW_HEIGHT / 2.0 + 100.0], c.transform, g);

    // Draw the curve (arc) using CircleArc
    let arc = CircleArc::new(WHITE, 1.0, std::f64::consts::PI * 3.0 / 2.0, 0.0);
    arc.draw([WINDOW_WIDTH / 2.0 - 100.0 - CURVE_RADIUS_LEFT, WINDOW_HEIGHT / 2.0 + 100.0 - CURVE_RADIUS_LEFT, CURVE_RADIUS_LEFT * 2.0, CURVE_RADIUS_LEFT * 2.0], &DrawState::default(), c.transform, g);

    // Draw the straight line from the curve's end point to the screen's right edge.
    line(WHITE, 1.0, [WINDOW_WIDTH / 2.0 - 100.0, WINDOW_HEIGHT / 2.0 - 50.0, 0.0, WINDOW_HEIGHT / 2.0 - 50.0], c.transform, g);
}

pub fn draw_south_left(c: Context, g: &mut G2d) {
    // Draw the straight line from the top to the curve's starting point.
    line(WHITE, 1.0, [WINDOW_WIDTH / 2.0 - 50.0, 0.0, WINDOW_WIDTH / 2.0 - 50.0, WINDOW_HEIGHT / 2.0 - 100.0], c.transform, g);
    ...
    // Draw the straight line from the curve's end point to the screen's right edge.
    line(WHITE, 1.0, [WINDOW_WIDTH / 2.0 + 100.0, WINDOW_HEIGHT / 2.0 + 50.0, WINDOW_WIDTH, WINDOW_HEIGHT / 2.0 + 50.0], c.transform, g);
}
